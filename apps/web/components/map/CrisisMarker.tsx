        ...
        <Popup className="custom-tactical-popup">
          <div className="min-w-[200px] bg-[#1e2427] text-[#CCCCCC] p-1 font-sans">
            <div className={`flex items-center gap-2 border-b border-white/10 pb-2 mb-2 ${getColor() === 'red' ? 'text-red-500' : 'text-orange-500'}`}>
                ...
                <h3 className="font-black uppercase tracking-tighter italic">{crisis.title}</h3>
            </div>
            
            <p className="text-[11px] leading-relaxed italic mb-3">"{crisis.description}"</p>
            
            <div className="grid grid-cols-2 gap-2 border-t border-white/5 pt-2">
                <div className="flex flex-col">
                    <span className="text-[8px] uppercase text-white/30 font-bold">Priorité</span>
                    <span className="text-[9px] font-black uppercase">{crisis.severity}</span>
                </div>
                <div className="flex flex-col">
                    <span className="text-[8px] uppercase text-white/30 font-bold">État</span>
                    <span className="text-[9px] font-black uppercase text-green-500">{crisis.status}</span>
                </div>
            </div>
