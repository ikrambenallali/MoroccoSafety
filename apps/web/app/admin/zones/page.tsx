...
import { Map, MapPin, Plus, Globe, Layers, ChevronRight } from 'lucide-react'
        ...
        <div className="p-8 bg-[#2B3337] min-h-screen text-[#CCCCCC]">
            ...
            <div className="flex justify-between items-center mb-10 border-b border-[#8E1616]/30 pb-6">
                ...
                <button
                    ...
                    className="bg-[#D84040] text-white px-6 py-3 rounded-sm flex items-center gap-2 font-black uppercase tracking-[0.2em] text-[10px] transition-all shadow-lg border border-white/10"
                >
                    <Plus size={16} /> Définir une Zone
                </button>
            </div>
                        ...
                        {zones.map((z: any) => {
                            ...
                            return (
                                <tr key={z._id} className="hover:bg-white/5 transition-colors group">
                                    ...
                                    {/* TYPE */}
                                    <td className="p-5">
                                        <div className="flex items-center gap-2 text-[10px] font-mono text-[#CCCCCC]/60 italic">
                                            <Layers size={12} />
                                            {z.geometry?.type || 'N/A'}
                                        </div>
                                    </td>
                                    ...
                                    <td className="p-5">
                                        {z.crisisId ? (
                                            <div className="inline-flex items-center gap-2 px-2 py-1 bg-[#8E1616]/10 border border-[#8E1616]/40 text-[#D84040] text-[10px] font-black uppercase">
                                                <Globe size={10} />
                                                {z.crisisId.title}
                                            </div>
                                        ) : (
                                            <span className="text-[10px] text-white/20 uppercase font-bold tracking-widest">Zone Libre</span>
                                        )}
                                    </td>
                        ...
                        {zones.length === 0 && (
                            <tr>
                                <td colSpan={5} className="p-20 text-center">
                                    <p className="text-[10px] uppercase tracking-[0.4em] text-white/20 font-bold italic">
            ...
            <div className="mt-6 flex justify-between items-center px-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] uppercase font-bold tracking-widest text-white/40">Projection: WGS 84 / Pseudo-Mercator</span>
                </div>
            </div>
        </div>
    )
